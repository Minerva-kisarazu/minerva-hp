...
const gradePolicies = [
  {
    ...
    body: '小学生のうちは、答えが合ったかどうかよりも、どうやって答えを出したかが大切です。分数が数として何を表しているのか、単位がなぜ変わるのか。ここが曖昧なまま計算のやり方だけを覚えると、中学以降で必ず止まります。国語も同じで、設問が何を求めているかを読むところから始めます。「なぜそうなるか」を考え、設問が何を聞いているかを読む習慣を、この段階から積み重ねます。',
    items: [
      '分数・単位・割合の意味の理解',
      '式や図を自分で書く習慣',
      '設問の指示を読んでから答える',
  ...
  },
  {
    ...
    body: '中学生の失点は、単元の理解不足だけが原因ではありません。途中式を書かないことで生まれる符号ミス、be動詞と一般動詞の混在、設問の読み落とし。こうした毎回同じ形で起きるミスを先に潰し、定期テストで取れるはずの点を取りこぼさない状態をつくります。あわせて、自分で問題を読み、必要な情報を整理し、解き方を選ぶ進め方を授業と自習で身につけます。原因が前の学年にある場合は、そこまで戻って埋め直します。',
    items: [
      '途中式を残す書き方の徹底',
      '英語は語順から組み立てる',
      '前学年にある原因の特定と復習',
  ...
  },
  {
    ...
    body: '高校では扱う範囲が一気に広がり、英語は長い文の構造、数学は公式の意味と適用条件が問われます。高校以降は、知識量だけでなく、初めて見る文章や問題を自分で読み解き、必要な知識を選んで使う力が求められます。「解説を読んで納得した」だけでは、入試や定期テストの初見問題では止まります。ミネルバでは、つまずいた箇所の知識・考え方・解き方を一緒に整理し、最後は何も見ずに答案を最後まで書けるところまで授業でつなげます。優先順位や1日の量の決め方は、その内容を自習で再現するための手段です。',
    items: [
      '英文は品詞・句と節から構造を取る',
      '数学は公式の意味と適用条件まで確認する',
...
const weeklySchedule: ScheduleDay[] = [
  {
    day: '月曜日',
    rows: [
      { time: '18:50〜20:10', activity: '英語授業（個別指導・読み方／解き方の修正）', isLesson: true },
      {
        time: '20:20〜20:50',
