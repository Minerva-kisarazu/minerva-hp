/**
 * report.csv から抽出した実指導報告。
 * 個人が特定されない範囲で原文の趣旨を保ち、カード表示用に見出し付けしている。
 * 存在しない指導内容は追加していない。
 */
export type ReportSample = {
  id: string;
  grade: string;
  subject: string;
  topic: string;
  stuck: string;
  guidance: string;
  next: string;
};

export const reportSamples: ReportSample[] = [
  {
    id: 'el5-kokugo',
    grade: '小学5年',
    subject: '国語',
    topic: '物語文の読解と、熟語の成り立ち',
    stuck:
      '設問の指示や指定字数を無視しがちで、選択肢も本文との違いを考えずに選んでしまう場面がありました。',
    guidance:
      '設問をきちんと読んでから答えるよう伝え、選択肢では「何が本文と違うのか」を考えさせるように指導しました。後半の熟語は、途中からほぼ間違えずに進められていました。',
    next: '設問指示の確認を、物語文の読解でも繰り返し意識させます。',
  },
  {
    id: 'el6-sansu',
    grade: '小学6年',
    subject: '算数',
    topic: '逆比・比例式の利用（文章題）',
    stuck:
      '分数の比を整数の比に直すところで誤りがあり、比の「1あたりの量(①)」が何に当たるかを考えられていませんでした。早く解くことを優先し、例題の解法をなぞって数字を当てはめる癖も残っています。',
    guidance:
      '逆比の変換と、比の「1あたりの量(①)」の意味を改めて確認し、文章題を解き直しました。一つ一つ確認すると何をしているか答えられることもあるため、そこを丁寧に確認しながら進めました。',
    next: '「解法をなぞる」のではなく、各数字の意味を言える状態まで、根気強く確認します。',
  },
  {
    id: 'jh1-english',
    grade: '中学1年',
    subject: '英語',
    topic: 'be going to・三単現・過去形などの基本文法の復習',
    stuck:
      'be going to が入ると be動詞と一般動詞の混同が見られ、主語が複数のときに三単現の s をつけてしまう、過去形に気づかない、といった細かい誤りも出ています。',
    guidance:
      'be動詞と一般動詞の区別を毎回確認するよう指導しました。大問ごとに丸付けし、間違えた理由を確認し、分からないところは印をつけて授業で聞く流れも伝えました。',
    next: '混同しやすい形は授業のたびに確認し、丸付けとやり直しの習慣を定着させます。',
  },
  {
    id: 'jh2-math',
    grade: '中学2年',
    subject: '数学',
    topic: '学校ワークの平面図形（扇形・中心角）',
    stuck:
      '基本の解き方や公式は覚えられており正答も多い一方、扇形と中心角は「なぜその解き方をするのか」を理解しないまま覚えた手順で解いている様子でした。計算ミスも見られました。',
    guidance:
      '間違えた問題・分からなかった問題について、根本から理解する時間を取りました。あわせて、計算を楽にする工夫も意識させています。',
    next: '「覚えている解き方」で終わらせず、理由まで言える状態を自習でも確認します。',
  },
  {
    id: 'jh2-kokugo',
    grade: '中学2年',
    subject: '国語',
    topic: '文章問題（記号・記述）',
    stuck:
      '記号問題は後半ほぼ正解できていましたが、記述は書けていても微妙にズレた解答がありました。',
    guidance:
      'まず設問と傍線部だけを見て簡潔に答え、その後字数に合わせて付け足す部分を探す、という順で考えるよう伝えました。',
    next: '読む力はあるため、注意を向けるポイントを定着させ、記述の安定につなげます。',
  },
  {
    id: 'jh3-english',
    grade: '中学3年',
    subject: '英語',
    topic: '不定詞の応用（It is 〜 for 人 to do / want 人 to do）',
    stuck:
      'It is 〜 for 人 to do の形はしっかり使えていましたが、want 人 to do では主語と目的語の混同がありました。人称代名詞やスペルのミスも残っています。',
    guidance:
      '書き始める前に、現在形か過去形か・肯定か否定か・平叙文か疑問文かを必ず確認するよう指示しました。語順で間違えない点は良いので、その強みを活かしつつ混同を整理しています。',
    next: '主語・目的語の関係と、時制・肯定否定の確認をセットで定着させます。',
  },
  {
    id: 'jh1-math',
    grade: '中学1年',
    subject: '数学',
    topic: '正負の数（分数・小数の計算を含む）',
    stuck:
      '正負の数の法則は理解できている一方、分数・小数の計算で通分と混同したり、約分のタイミングが逆になっていたりしました。乗算では符号と足し算・引き算の記号が混ざってしまう場面もありました。',
    guidance:
      '分数・小数のやり方を一緒に確認しました。指導した内容はしっかり使えているので、区別を意識させながら進めています。',
    next: '正負の符号と計算記号の区別を、次回授業で改めて確認します。',
  },
  {
    id: 'el4-sansu',
    grade: '小学4年',
    subject: '算数',
    topic: '概数での計算',
    stuck:
      '概数になって進めやすくなった一方、途中の計算をメモせず消してしまうため、間違えたときに再度計算し直す必要が出ていました。',
    guidance:
      'ノートを贅沢に使ってどんどん書き加えていくよう指導しました。計算トレーニングとテキストの進め方の目安も宿題として伝えました。',
    next: '途中式を残す習慣を、次回以降の計算単元でも確認します。',
  },
  {
    id: 'hs3-english',
    grade: '高校3年',
    subject: '英語',
    topic: '英文の構造分析（句・節、過去分詞の後置修飾）',
    stuck:
      '句や節は正確に把握できるようになってきた一方、過去分詞の後置修飾に気づけないことが多くあります。',
    guidance:
      '（述語）動詞ではないのに過去分詞形が出てきたら、基本は修飾語だと判断させました。同格や省略などの特殊構文も、出てきたものから少しずつ覚えていく方針です。',
    next: '後置修飾と特殊構文を、構造分析の練習の中で積み重ねます。',
  },
  {
    id: 'hs3-math',
    grade: '高校3年',
    subject: '数学',
    topic: '自学範囲の確認と、次単元の要点解説',
    stuck:
      '「解説を読んで納得した」段階と、「何も見ずに答案を書き切れた」段階が混同されやすい状態です。',
    guidance:
      '「問題文以外何も見ずに、最初から最後まで答案を書けた」状態を、その問題をやった基準として共有しました。不明点の解消→次範囲の解説、の流れで進めています。',
    next: '次回も同じ到達基準で、不明点解消から次の範囲へ進めます。',
  },
  {
    id: 'el6-kokugo',
    grade: '小学6年',
    subject: '国語',
    topic: '物語文の読解',
    stuck:
      '人物関係がややこしい文章では把握しきれず、設問の指示を軽視する傾向も見られました。',
    guidance:
      '今後の物語文では人物関係図を作るよう指導し、設問指示は必ず確認するよう繰り返し伝えています。',
    next: '人物関係の整理と設問確認を、読解の前工程として定着させます。',
  },
  {
    id: 'jh2-math-formula',
    grade: '中学2年',
    subject: '数学',
    topic: '三学期期末のやり直し（作図・扇形・反比例）',
    stuck:
      '作図はやり方の雰囲気は知っていても「なぜそうするのか」が分かっておらず覚えきれていませんでした。扇形では分数計算の誤り、反比例もなんとなくの手順覚えが原因でした。',
    guidance:
      '授業の中で説明を受け、自分で説明できるようにする習慣を付ける方針を共有しました。計算が不安定な部分はドリル等での対策も検討が必要、と伝えています。',
    next: '「なぜ解けるのか」を自分の言葉で言えるかを、類題で確認します。',
  },
];
